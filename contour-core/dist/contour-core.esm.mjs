    ...
    function closeBoundaries(pathinfo, contours) {
      ...
      switch (contours.type || contours.coloring) {
        ...
        case "fill":
          ...
          for (i = 0; i < nb; i++) {
            if (z[i][0] !== null) {
              if (z[i][0] < boundaryMin)
                boundaryMin = z[i][0];
              if (z[i][0] > boundaryMax)
                boundaryMax = z[i][0];
            }
            if (z[i][na - 1] !== null) {
              if (z[i][na - 1] < boundaryMin)
                boundaryMin = z[i][na - 1];
              if (z[i][na - 1] > boundaryMax)
                boundaryMax = z[i][na - 1];
            }
          }
          for (i = 1; i < na - 1; i++) {
            if (z[0][i] !== null) {
              if (z[0][i] < boundaryMin)
                boundaryMin = z[0][i];
              if (z[0][i] > boundaryMax)
                boundaryMax = z[0][i];
            }
            if (z[nb - 1][i] !== null) {
              if (z[nb - 1][i] < boundaryMin)
                boundaryMin = z[nb - 1][i];
              if (z[nb - 1][i] > boundaryMax)
                boundaryMax = z[nb - 1][i];
            }
          }
          ...
          for (i = 0; i < pathinfo.length; i++) {
            ...
            pi.prefixBoundary = !pi.edgepaths.length && (boundaryMin > pi.level || allDataBelow && i === 0 || pi.starts.length && boundaryMin === pi.level);
          }
          break;
        case "constraint":
  ...
  "null_handling/clip_mask.js"(exports, module) {
    ...
    function generateNullMaskPolygons(contourResult, options) {
      ...
      if (dataX && dataY) {
        ...
        for (var i = 0; i < n; i++)
          clipPathInfo.x.push(i / scale);
        for (var j = 0; j < m; j++)
          clipPathInfo.y.push(j / scale);
      }
      marchingSquares.makeCrossings([clipPathInfo]);
      ...
      var tol = Math.max(1e-10, Math.max(xRange, yRange) * 1e-3);
...
var require_null_handling = __commonJS({
  "null_handling/index.js"(exports, module) {
    ...
    module.exports = {
      ...
      generateNullMaskPolygons: require_clip_mask().generateNullMaskPolygons,
  ...
  "overlay/services/interactive_drawer.js"(exports, module) {
    ...
    InteractiveDrawer.prototype = {
      ...
      start: function(mode, options, canvas) {
        ...
        this._bindEvents();
        this._events.emit(Events.START, { mode });
      },
      /**
       * 停止交互绘制
       */
      ...
      _transitionToIdle: function(reason) {
        ...
        this._mousePos = null;
        this._canvas = null;
        this._events.emit(Events.STOP, { reason });
        this._refresh();
      },
      /**
       * 完成绘制并转换状态
  ...
  "geojson.js"(exports, module) {
    ...
    function toNullMaskGeoJSON(result, options) {
      ...
      var polygonData = nullHandling.generateNullMaskPolygons(result, {
        ...
        dataY: result.pathinfo && result.pathinfo[0] ? result.pathinfo[0].y : void 0,
